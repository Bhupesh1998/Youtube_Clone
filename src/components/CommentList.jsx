    ...
    <div className="flex gap-2 sm:gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm flex-wrap">
          ...
          <span className="text-gray-500 text-[10px] sm:text-xs">{timestamp}</span>
        </div>
        <p className="mt-1 text-xs sm:text-sm text-gray-800">{text}</p>
        <div className="flex items-center gap-2 sm:gap-4 mt-1">
          <button className="flex items-center gap-0.5 sm:gap-1 text-gray-500 hover:text-gray-700">
          ...
          <button className="text-[10px] sm:text-xs font-medium text-gray-500 hover:text-gray-700">
            Reply
          </button>
        </div>
      </div>
  ...
  return comments.map((comment, index) => (
    <div key={index}>
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="ml-6 sm:ml-12 pl-3 sm:pl-6 border-l-2 border-gray-100 mt-2 sm:mt-3 space-y-3 sm:space-y-4">
